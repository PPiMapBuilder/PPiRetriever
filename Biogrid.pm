...
while (<gene_name_to_uniprot_file>) {
	...
	$hash_orga_tax->{@convertion_data[0]}->{@convertion_data[2]} = @convertion_data[1];
}
...
while (<data_file>) {
	...
	foreach $tax_id (keys %hash_orga_tax) {
		if (@data[16] eq $tax_id) { # If the taxonomy id is present 
			$origin=$hash_orga_tax{$tax_id}; # We retrieve the organism name
	...
	$interaction = Interaction->new(\@A, \@B, $origin, $database, \@pubmed, \@sys_exp);
